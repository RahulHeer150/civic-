...
const HowItWorks = () => {
  const items = [
    {
      icon: <FaLaptop className="text-[3rem] text-blue-950 mb-4 justify-center" />, 
      title: "Report", 
    ...
    <div className="p-4 text-center mt-20">
      ...
      <div className="flex flex-wrap justify-center gap-8">
        {items.map((item, index) => (
          <div key={index} className="bg-[#fff] border-1 border-[#dcdde1] rounded-lg p-6 max-w-[300px] text-center items-center">
            {item.icon}
           
            <div className="text">
               <h2 className="text-3xl font-bold text-blue-950 mb-2">{item.title}</h2>
              <p className="text-lg text-[#636e72]">{item.description}</p>
            </div>
          </div>
        ))}
      </div>
      <div className="mt-12">
        <h2 className="text-2xl text-blue-600 mb-2">Join Us</h2>
        <p className="text-md px-10 m-auto md:py-4">Be part of a proactive community committed to making impactful changes. Whether you're reporting issues, voting on priorities, or tracking progress, every step you take contributes to building a better future.</p>
        <h2 className="text-2xl text-[#2c3e50] mb-2">Why It Matters</h2>
        <p className="text-md px-10 m-auto md:py-4">Your involvement ensures that essential issues are addressed promptly and effectively. By working together, we can create transparent and actionable solutions for everyone.</p>
      </div>
    </div>
  );
};
